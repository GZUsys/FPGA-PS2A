...
//新增参考序列分段函数
void create_ref_segments(refsynth_t *ref) {  
    ref->num_segments = (uint32_t*)malloc(sizeof(uint32_t) * ref->num_ref);  
    ref->segment_lengths = (int32_t**)malloc(sizeof(int32_t*) * ref->num_ref);  
    ref->forward_segments = (float***)malloc(sizeof(float**) * ref->num_ref);  
      
    int8_t has_reverse = (ref->reverse != NULL);  
    if(has_reverse) {  
        ref->reverse_segments = (float***)malloc(sizeof(float**) * ref->num_ref);  
    }  
      
    for(int i = 0; i < ref->num_ref; i++) {  
        int32_t total_len = ref->ref_lengths[i];  
        ...
        if(total_len <= SEGMENT_SIZE) {  
            ...
            int32_t remaining = total_len - SEGMENT_SIZE;  
            int32_t step = SEGMENT_SIZE - OVERLAP_SIZE;  
            num_segs = (uint32_t)(1 + (remaining + step - 1) / step);  
        }  
          
        ref->num_segments[i] = num_segs;  
        ref->segment_lengths[i] = (int32_t*)malloc(sizeof(int32_t) * num_segs);  
        ref->forward_segments[i] = (float**)malloc(sizeof(float*) * num_segs);  
          
        if(has_reverse) {  
            ref->reverse_segments[i] = (float**)malloc(sizeof(float*) * num_segs);  
        }  
          
        // 生成每个分段  
        for(int seg = 0; seg < num_segs; seg++) {  
            int32_t seg_start = seg * (SEGMENT_SIZE - OVERLAP_SIZE);  
            int32_t seg_len = (seg_start + SEGMENT_SIZE <= total_len) ? SEGMENT_SIZE : total_len - seg_start;  
              
            // 分段总长度 = 1(ID) + seg_len(信号)  
            //int32_t total_seg_len = seg_len + 2;  
            ref->segment_lengths[i][seg] = seg_len;//total_seg_len;  
              
            // 分配并填充正链  
            ref->forward_segments[i][seg] = (float*)malloc(sizeof(float) * seg_len);//total_seg_len);  
            //ref->forward_segments[i][seg][0] = (float)seg;  // 第一个元素为参考序列索引  
            //ref->forward_segments[i][seg][1] = 0;  // 第二个元素为0  
            memcpy(&ref->forward_segments[i][seg][0], ref->forward[i] + seg_start, seg_len * sizeof(float));  
              
            // 反链处理  
            if(has_reverse) {  
                ref->reverse_segments[i][seg] = (float*)malloc(sizeof(float) * seg_len);//(total_seg_len-2));  
                //ref->reverse_segments[i][seg][0] = (float)i;
                //ref->reverse_segments[i][seg][1] = 0;   
                memcpy(&ref->reverse_segments[i][seg][0], ref->reverse[i] + seg_start, seg_len * sizeof(float));  
            }  
              
            fprintf(stderr, "Created segment %d: ID=%d, Start=%d, Length=%d\n", seg, seg, seg_start, seg_len);  
        }  
    }  
}  
