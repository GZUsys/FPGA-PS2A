...
//新增分段逻辑
#define SEGMENT_SIZE_KB 50  
#define SEGMENT_SIZE (SEGMENT_SIZE_KB * 1024/4/2)  // 51200个浮点数(4个字节为1个数量，双链除2)  
#define OVERLAP_SIZE 200  // 重叠区域  
//新增结束
...
typedef struct {
    ...
    float **reverse;
    //segmented cache
    uint32_t *num_segments; //The number of segments for each reference sequence
    int32_t **segment_lengths;    // Each segment length (including the ID element) 
    float ***forward_segments;    // [ref][segment][ID+signal]  
    float ***reverse_segments;    // Segmentation of the reverse strand in the DNA pattern 
    //
} refsynth_t;
