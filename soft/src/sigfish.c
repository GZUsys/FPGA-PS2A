...
aln_t *init_aln(){
    ...
    int32_t rid = -1;
    char d = 0;
    for (int l=0; l<SECONDARY_CAP;l++) aln[l] = (aln_t){rid,pos,pos,score,score2,d,0};
...
void dtw_fpga(core_t* core,db_t* db){
    ...
    double dtw_fpga_star = realtime();
    int32_t dc1_load_num,dc2_load_num;
    dc1_load_num = core->ref->num_segments[0]/2 + core->ref->num_segments[0]%2;
    dc2_load_num = core->ref->num_segments[0]/2;
    haru_load_ref1_num(core->haru, dc1_load_num);
    haru_load_ref2_num(core->haru, dc2_load_num);
    int32_t qlen = core->opt.query_size;
    int32_t i=0;
    int32_t rlen = 0;
    for (i = 0; i < db->n_rec; i += 8) {
        //Extract query batches
        int32_t *query_r = (int32_t *)malloc(sizeof(int32_t)*(qlen+2)*8);
        MALLOC_CHK(query_r);
        //aln_t *aln=init_aln();
        for(int j = 0; j < 8; j++){
            if((i+j) < db->n_rec){
                if(db->slow5_rec[i+j]->len_raw_signal>0 && db->et[i+j].n>0){
                    int64_t start_idx = db->qstart[i+j];
                    int64_t end_idx = db->qend[i+j];
                    int64_t n =  db->et[i+j].n;
                    int8_t rna = core->opt.flag & SIGFISH_RNA;
                    query_r[j*(qlen+2)]=i+j;//queryID
                    query_r[j*(qlen+2)+1]=0;
                    int32_t *query = &query_r[j*(qlen+2)+2];
                    for(int k=0;k<qlen;k++){
                        if (!(core->opt.flag & SIGFISH_INV) && rna){
                            query[qlen-1-k] = (int32_t) (db->et[i+j].event[k+start_idx].mean * 32);
                        }
                        else{
                            query[k] = (int32_t) (db->et[i+j].event[k+start_idx].mean * 32);
                        }
                    }
        ...
        //Load the query into the PL
        haru_process_query1(core->haru, query_r, 8*(qlen+2));
        haru_process_query2(core->haru, query_r, 8*(qlen+2));

        free(query_r);
        
        //Load the ref sequence in sequence
        for(int m = 0;m < core->ref->num_segments[0]; m++){
            rlen = core->ref->segment_lengths[0][m];
            int32_t *ref_r = (int32_t *)malloc(sizeof(int32_t) * (rlen*2 + 2));
            MALLOC_CHK(ref_r);
            ...
            for(int y=0;y<rlen;y++){
                ref[y] = (int32_t) (core->ref->forward_segments[0][m][y] * 32);
                ref[y+rlen] = (int32_t) (core->ref->reverse_segments[0][m][y] *32);   
            }
            if(m % 2 == 0){
                haru_load_reference1(core->haru, ref_r, (rlen * 2) + 2);
            }else{
                haru_load_reference2(core->haru, ref_r, (rlen * 2) + 2);
            }
            free(ref_r);
           
            //Retrieve the previous processing result    
            if(m == 1 && i > 0){
                search_result_t results;
                haru_get_results(core->haru,&results);
                  //ps post_process
                if(results.rid1 == core->ref->num_segments[0]-1){
                    rlen = core->ref->segment_lengths[0][core->ref->num_segments[0]-1];
                }else{
                    rlen = core->ref->segment_lengths[0][0];
                }
                db->aln[i-8].score = ((float)results.score1)/32.0;
                db->aln[i-8].score2 = ((float)results.score1)/32.0;
                if (results.position1 > rlen-2) {
                    // reverse
                    db->aln[i-8].pos_st = (results.rid1 * (SEGMENT_SIZE - OVERLAP_SIZE)) + (2*rlen-4) - results.position1;
                    db->aln[i-8].pos_end = db->aln[i-8].pos_st + 200;
                    db->aln[i-8].d = '-';
                } else {
                    // forward
                    db->aln[i-8].pos_end = (results.rid1 * (SEGMENT_SIZE - OVERLAP_SIZE)) + results.position1;
                    db->aln[i-8].pos_st = results.position1 - 200;
                    db->aln[i-8].d = '+';
                }
                db->aln[i-8].rid = 0;
                db->aln[i-8].mapq = 60;

                if(results.rid2 == core->ref->num_segments[0]-1){
                    rlen = core->ref->segment_lengths[0][core->ref->num_segments[0]-1];
                }else{
                    rlen = core->ref->segment_lengths[0][0];
                }
                db->aln[i-7].score = ((float)results.score2)/32.0;
                db->aln[i-7].score2 = ((float)results.score2)/32.0;
                if (results.position2 > rlen-2) {
                        // reverse
                    db->aln[i-7].pos_st = (results.rid2 * (SEGMENT_SIZE - OVERLAP_SIZE)) + (2*rlen-4) - results.position2;
                    db->aln[i-7].pos_end = db->aln[i-7].pos_st + 200;
                    db->aln[i-7].d = '-';
                } else {
                    // forward
                    db->aln[i-7].pos_end = (results.rid2 * (SEGMENT_SIZE - OVERLAP_SIZE)) + results.position2;
                    db->aln[i-7].pos_st = results.position2 - 200;
                    db->aln[i-7].d = '+';
                }
                db->aln[i-7].rid = 0;
                db->aln[i-7].mapq = 60;
                
                if(results.rid3 == core->ref->num_segments[0]-1){
                    rlen = core->ref->segment_lengths[0][core->ref->num_segments[0]-1];
                }else{
                    rlen = core->ref->segment_lengths[0][0];
                }
                db->aln[i-6].score = ((float)results.score3)/32.0;
                db->aln[i-6].score2 = ((float)results.score3)/32.0;
                if (results.position3 > rlen-2) {
                    // reverse
                    db->aln[i-6].pos_st = (results.rid3 * (SEGMENT_SIZE - OVERLAP_SIZE)) + (2*rlen-4) - results.position3;
                    db->aln[i-6].pos_end = db->aln[i-6].pos_st + 200;
                    db->aln[i-6].d = '-';
                } else {
                    // forward
                    db->aln[i-6].pos_end = (results.rid3 * (SEGMENT_SIZE - OVERLAP_SIZE)) + results.position3;
                    db->aln[i-6].pos_st = results.position3 - 200;
                    db->aln[i-6].d = '+';
                }
                db->aln[i-6].rid = 0;
                db->aln[i-6].mapq = 60;
                ...
                if(results.rid4 == core->ref->num_segments[0]-1){
                    rlen = core->ref->segment_lengths[0][core->ref->num_segments[0]-1];
                }else{
                    rlen = core->ref->segment_lengths[0][0];
                }
                db->aln[i-5].score = ((float)results.score4)/32.0;
                db->aln[i-5].score2 = ((float)results.score4)/32.0;
                if (results.position4 > rlen-2) {
                        // reverse
                    db->aln[i-5].pos_st = (results.rid4 * (SEGMENT_SIZE - OVERLAP_SIZE)) + (2*rlen-4) - results.position4;
                    db->aln[i-5].pos_end = db->aln[i-5].pos_st + 200;
                    db->aln[i-5].d = '-';
                } else {
                    // forward
                    db->aln[i-5].pos_end = (results.rid4 * (SEGMENT_SIZE - OVERLAP_SIZE)) + results.position4;
                    db->aln[i-5].pos_st = results.position4 - 200;
                    db->aln[i-5].d = '+';
                }
                db->aln[i-5].rid = 0;
                db->aln[i-5].mapq = 60;
                ...
                if(results.rid5 == core->ref->num_segments[0]-1){
                    rlen = core->ref->segment_lengths[0][core->ref->num_segments[0]-1];
                }else{
                    rlen = core->ref->segment_lengths[0][0];
                }
                db->aln[i-4].score = ((float)results.score5)/32.0;
                db->aln[i-4].score2 = ((float)results.score5)/32.0;
                if (results.position5 > rlen-2) {
                    // reverse
                    db->aln[i-4].pos_st = (results.rid5 * (SEGMENT_SIZE - OVERLAP_SIZE)) + (2*rlen-4) - results.position5;
                    db->aln[i-4].pos_end = db->aln[i-4].pos_st + 200;
                    db->aln[i-4].d = '-';
                } else {
                    // forward
                    db->aln[i-4].pos_end = (results.rid5 * (SEGMENT_SIZE - OVERLAP_SIZE)) + results.position5;
                    db->aln[i-4].pos_st = results.position5 - 200;
                    db->aln[i-4].d = '+';
                }
                db->aln[i-4].rid = 0;
                db->aln[i-4].mapq = 60;
                ...
                if(results.rid6 == core->ref->num_segments[0]-1){
                    rlen = core->ref->segment_lengths[0][core->ref->num_segments[0]-1];
                }else{
                    rlen = core->ref->segment_lengths[0][0];
                }
                db->aln[i-3].score = ((float)results.score6)/32.0;
                db->aln[i-3].score2 = ((float)results.score6)/32.0;
                if (results.position6 > rlen-2) {
                    // reverse
                    db->aln[i-3].pos_st = (results.rid6 * (SEGMENT_SIZE - OVERLAP_SIZE)) + (2*rlen-4) - results.position6;
                    db->aln[i-3].pos_end = db->aln[i-3].pos_st + 200;
                    db->aln[i-3].d = '-';
                } else {
                    // forward
                    db->aln[i-3].pos_end = (results.rid6 * (SEGMENT_SIZE - OVERLAP_SIZE)) + results.position6;
                    db->aln[i-3].pos_st = results.position6 - 200;
                    db->aln[i-3].d = '+';
                }
                db->aln[i-3].rid = 0;
                db->aln[i-3].mapq = 60;
                ...
                if(results.rid7 == core->ref->num_segments[0]-1){
                    rlen = core->ref->segment_lengths[0][core->ref->num_segments[0]-1];
                }else{
                    rlen = core->ref->segment_lengths[0][0];
                }
                db->aln[i-2].score = ((float)results.score7)/32.0;
                db->aln[i-2].score2 = ((float)results.score7)/32.0;
                if (results.position7 > rlen-2) {
                    // reverse
                    db->aln[i-2].pos_st = (results.rid7 * (SEGMENT_SIZE - OVERLAP_SIZE)) + (2*rlen-4) - results.position7;
                    db->aln[i-2].pos_end = db->aln[i-2].pos_st + 200;
                    db->aln[i-2].d = '-';
                } else {
                    // forward
                    db->aln[i-2].pos_end = (results.rid7 * (SEGMENT_SIZE - OVERLAP_SIZE)) + results.position7;
                    db->aln[i-2].pos_st = results.position7 - 200;
                    db->aln[i-2].d = '+';
                }
                db->aln[i-2].rid = 0;
                db->aln[i-2].mapq = 60;
                ...
                if(results.rid8 == core->ref->num_segments[0]-1){
                    rlen = core->ref->segment_lengths[0][core->ref->num_segments[0]-1];
                }else{
                    rlen = core->ref->segment_lengths[0][0];
                }
                db->aln[i-1].score = ((float)results.score8)/32.0;
                db->aln[i-1].score2 = ((float)results.score8)/32.0;
                if (results.position8 > rlen-2) {
                    // reverse
                    db->aln[i-1].pos_st = (results.rid8 * (SEGMENT_SIZE - OVERLAP_SIZE)) + (2*rlen-4) - results.position8;
                    db->aln[i-1].pos_end = db->aln[i-1].pos_st + 200;
                    db->aln[i-1].d = '-';
                } else {
                    // forward
                    db->aln[i-1].pos_end = (results.rid8 * (SEGMENT_SIZE - OVERLAP_SIZE)) + results.position8;
                    db->aln[i-1].pos_st = results.position8 - 200;
                    db->aln[i-1].d = '+';
                }
                db->aln[i-1].rid = 0;
                db->aln[i-1].mapq = 60;
